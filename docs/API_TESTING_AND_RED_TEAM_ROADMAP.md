...
| Event and indexer projection proof | Blocked — `28/260` real writes proven on 2026-08-10 | generated-registry decode, PostgreSQL projection, replay/reorg/rollback resilience, deterministic receipts for the remaining `232` writes, and a production call-tracer capability decision |
...
- The last verified gates passed `51/51` active indexer-assurance tests, `4/4` disposable-PostgreSQL tests, root TypeScript validation, lint, build, `coverage:check` (`492` functions, `218` events, `492` HTTP methods, `260/260` write invariants), and measured coverage at `99.73%` statements, `99.61%` branches, `99.54%` functions, and `99.80%` lines.
...
- The local proof RPC reported `debug_traceTransaction` with `callTracer` as unsupported. Prove support on the configured production RPC or provide a dedicated trace endpoint before nested shared-signature events can be guaranteed to project rather than remain raw candidate evidence.
