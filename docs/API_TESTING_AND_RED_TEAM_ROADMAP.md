...
| Event and indexer projection proof | Blocked — `59/260` cumulative real writes proven on 2026-08-24 | generated-registry decode, PostgreSQL projection, replay/reorg/rollback resilience, and deterministic receipts for the remaining `201` writes; `VoiceAssetFacet.registerVoiceAssetForCaller` is mounted publicly but contract-restricted to an unreachable diamond self-call |
...
### Event And Indexer Projection Run — 2026-08-24
...
- The runner indexed all three additions with zero receipt failures. `VoiceClassificationsUpdated`, `ClassificationCategoryUpdated`, and `GeographicDataUpdated` each decoded through the generated registry as the event declared by its write invariant. The current cold artifact contains `132` receipts across `59` distinct methods, `202` canonical raw events, and `137` projection rows; replay left `raw_events` and all `22` projection-table counts unchanged. Cumulative real-receipt coverage is therefore `59/260`.
...
- The latest cold run passed all `10/10` stages on their first attempt and indexed `132` workflow receipts across `59` distinct catalog write methods. The production indexer persisted `202` canonical raw events and `137` projection rows, satisfied every `all`, `one-of`, or `none` event assertion plus every declared projection-table assertion, and preserved all `23` table counts when all proven blocks were replayed.
...
- Current verification is green: `pnpm run test:write-invariants` passed `5/5`, `pnpm run test:indexer:assurance` passed `57/57` active tests, `pnpm run test:indexer:postgres` passed `4/4`, `pnpm run test:local-fork-runner` passed `11/11`, and the isolated metadata receipt proof passed `1/1`. With `pnpm` selected from `pnpm-lock.yaml`, the ordered TypeScript, lint, and build gates passed, followed by an explicit `pnpm run coverage:check` at `492` functions, `218` events, `492` HTTP methods, and `260/260` write invariants.
- The 2026-08-24 loopback orchestrator completed all `10/10` stages on their first attempt. Fixture provisioning, the expanded HTTP receipt stage, marketplace settlement, governance activation/voting, the event-indexer stage, and the exhaustive sweep all passed; the final report contains only the established `16` structured `needs fixture` read/event gaps and no runner failure.

Blockers and next steps:

- Add deterministic fixtures and persisted receipt evidence for the remaining `201` write methods before claiming every write path is proven end to end.
