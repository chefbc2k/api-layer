...
| Write-method invariant metadata | Complete and verified (2026-08-25) | `260/260` ABI writes in `reviewed/reviewed-write-invariants.json`, stale/missing/signature/reference gates, `scripts/write-invariants-lib.test.ts` (`5/5` passing), green TypeScript/lint/build gates, and green `pnpm run coverage:check` |
| Actor and signer negative paths | Complete and verified (2026-08-25) | `259/259` mounted HTTP writes across `13` domains, `1,813` actor/method cases, `777` API-boundary cases, `3,150` role-lifecycle cases after the diamond-self-call correction, `100/100` focused tests, and green full/coverage gates |
