...
| Event and indexer projection proof | Blocked — `56/260` cumulative real writes proven on 2026-08-19 | generated-registry decode, PostgreSQL projection, replay/reorg/rollback resilience, and deterministic receipts for the remaining `204` writes; production call tracing was proven on 2026-08-11 |
...
### Event And Indexer Projection Run — 2026-08-19

Status: **blocked; do not merge this section yet**.

Evidence on `codex/event-indexer-proof`:

- The 2026-08-19 receipt expansion adds seven previously unproven writes: `MarketplaceFacet.updateListingPrice`, `VoiceAssetFacet.recordUsage`, `VoiceAssetFacet.lockVoiceAsset`, `VoiceAssetFacet.unlockVoiceAsset`, `VoiceAssetFacet.approveVoiceAsset`, and both `VoiceAssetFacet.safeTransferFrom` overloads. The marketplace fixture now advances through the contract's five-block repricing cooldown and requires the command, receipt, state readback, and `ListingPriceUpdated` query to succeed. A disposable voice asset proves usage count, reversible lock state, token approval, and ownership changes across both safe-transfer routes.
- The runner-owned Anvil artifact indexed all seven additions with zero receipt failures. Each expected event decoded through the generated registry and each receipt projected into `market_listings` or `voice_assets`; replay left all tracked table counts unchanged. Combined with the prior `49`-method cold artifact, cumulative real-receipt coverage is `56/260`. Because the runner retries replace the per-run HTTP receipt file, the latest standalone artifact contains `96` receipts across `50` methods, `143` canonical raw events, and `94` projection rows rather than the cumulative union.
- The local-fork runner has a tenth `event-indexer-proof` stage. The preceding `19/19` HTTP contract stage now persists all `82` successful receipt-confirmed transaction hashes instead of discarding them; the indexer proof combines that artifact with the lifecycle reports, keeps only successful writes to the configured diamond, attributes each selector through the generated write registry, and ingests each exact receipt block through `EventIndexer` into disposable PostgreSQL with all three migrations applied.
- The latest cold run passed all `10/10` stages and indexed `120` workflow receipts across `49` distinct catalog write methods. It added reversible receipt evidence for marketplace pause/unpause, voice-asset default royalty, default platform fee, registration pause, and the dataset maximum-assets setting, restoring every mutated value after its assertion. The production indexer persisted `188` canonical raw events and `126` projection rows, satisfied every `all`, `one-of`, or `none` event assertion plus every declared projection-table assertion, and preserved all `23` table counts when all proven blocks were replayed.
...
- Current verification is green: `pnpm run test:write-invariants` passed `5/5`, `pnpm run test:indexer:assurance` passed `57/57` active tests, `pnpm run test:indexer:postgres` passed `4/4`, `pnpm run test:local-fork-runner` passed `9/9`, TypeScript/lint/build passed in order, and the explicit `pnpm run coverage:check` remained complete at `492` functions, `218` events, `492` HTTP methods, and `260/260` write invariants.
- Measured coverage passed at `99.70%` statements, `99.55%` branches, `99.54%` functions, and `99.76%` lines. The runner initially lost a shared loopback RPC and the default Base Sepolia endpoint later reported no healthy backend; the successful evidence above came from a clean runner-owned Anvil on port `18548` forked from the publicnode Base Sepolia endpoint, so neither transient infrastructure fault was accepted as workflow evidence.
- The 2026-08-19 clean-fork orchestrator still ended with `23` aggregate gaps even though its `event-indexer-proof` stage passed. The HTTP stage retried after provider timeouts, the remaining-layer stage saw `ECONNRESET`, marketplace purchase exhausted its fixture actor's gas balance, governance activation timed out, and `19` fixture/event reads remained unresolved. These failures prevent treating this run as a clean all-stage promotion artifact.

Blockers and next steps:

- Add deterministic fixtures and persisted receipt evidence for the remaining `204` write methods before claiming every write path is proven end to end.
- Make retry receipt evidence cumulative and stabilize the fork provider, marketplace buyer funding, and governance activation so a single clean `10/10` orchestrator run preserves the full method union.
