...
- Standard TypeScript coverage currently reports `99.80%` lines, `99.73%` statements, `99.61%` branches, and `99.54%` functions across the measured API/client/indexer/script surface; the receipt-proof orchestration remains deliberately measurable alongside the API/client/indexer runtime.
...
| Event and indexer projection proof | Blocked — `28/260` real writes proven on 2026-08-10 | event decode plus indexer projection tests for each write workflow, including replay/reorg cases |
| Local-fork destructive automation | Complete and verified (2026-08-10) | deterministic `10/10`-stage cold run, funded/approved/aged fixtures, `18/18` HTTP contract proof, `38` real receipts indexed into PostgreSQL, `446/446` reviewed read/event attempts with structured state gaps, bounded Anvil history, strict live flags, `9/9` focused tests, and green TypeScript/lint/build/coverage gates |
...
### Event And Indexer Projection Run — 2026-08-10

Status: **blocked; do not merge this section yet**.

Evidence added on `codex/event-indexer-proof`:

- The merged local-fork runner now includes a tenth `event-indexer-proof` stage. It recursively discovers the prior workflow stages' real transaction hashes, filters successful writes to the configured diamond, attributes all writes from the unique generated calldata selector registry, and ingests each exact receipt block through `EventIndexer` into a disposable PostgreSQL database with all three migrations applied.
- A fresh cold run passed all `10/10` stages on the first attempt. The new stage found and indexed `38` successful workflow receipts across `28` distinct catalog write methods, persisted `68` canonical raw events and `53` projection rows, and matched every receipt against its `all`, `one-of`, or `none` event mode plus every declared projection table.
- The stage re-ingested every proven block and compared `raw_events` plus all `22` projection table counts before and after replay. Every count remained stable, providing real-receipt idempotency evidence in addition to the existing synthetic duplicate-log and PostgreSQL constraint tests.
- The persisted `.runtime/local-fork-proofs/event-indexer.json` artifact records the `28` proven method keys and the exact `232`-method remainder. This artifact deliberately keeps the workstream blocked rather than promoting catalog-wide synthetic proof to real write coverage.
- The generated-registry assurance test synthesizes and decodes logs for all `214` addressable event registry entries derived from the `218` ABI event declarations. The write-bound test consumes all `260` generated write invariants and proves `287` declared write-to-event expectations (`189` unique events), `150` declared write projection references, and `27` intentionally eventless writes.
- All `260` mounted write selectors are unique. The indexer now reads an ambiguous log's originating transaction selector and resolves it only when that write's generated invariant metadata names exactly one candidate; otherwise it persists all candidates and skips projection. This covers the `14` registry entries sharing `6` diamond-level signatures without arbitrary facet selection.
- Ambiguous events emitted by nested multisig, timelock, or generic executor calls can now be attributed from `debug_traceTransaction` call-trace selectors. If tracing is unavailable or multiple nested candidates remain, the indexer preserves the full candidate evidence and skips unsafe projection.
- Raw event insertion, every projection in the fetched range, and checkpoint advancement now share one database transaction. The two-log partial-failure test proves a failure on the second projection prevents checkpoint advancement and rolls the whole range back.
- A canonical `indexer_blocks` journal now records every fetched block, including empty blocks. Reorg detection searches the journal for the common ancestor, atomically orphans all divergent raw events, projections, and block rows, rebuilds current projections, rewinds the checkpoint, and resumes backfill from the rewound in-memory cursor rather than the stale pre-reorg cursor.
...
- `pnpm run test:indexer:assurance` passes `51/51` active tests, with the separately gated PostgreSQL file's `4` tests skipped in the unit command. Coverage includes receipt-artifact discovery and selector attribution, duplicate replay, delayed RPC, two-log partial-range failure, direct and nested transaction-context event resolution, deep common-ancestor reorg recovery, empty-block journaling, generated-registry decoding, and reviewed projection SQL.
- `pnpm run test:indexer:postgres` starts disposable PostgreSQL, applies all three migrations twice, and passes `4/4` real-database tests for unique-index replay idempotency, atomic raw/projection rollback, reorg orphaning plus current-row rebuild, and canonical block replacement after orphaning.
- Repository-wide instrumentation passes at `99.73%` statements, `99.61%` branches, `99.54%` functions, and `99.80%` lines. The indexer worker reports `96.52%` statements, `90.36%` branches, `92.59%` functions, and `96.29%` lines after exposing the bounded receipt-ingestion lifecycle.
- `pnpm exec tsc -p tsconfig.json --noEmit`, `pnpm run lint`, and the full `pnpm run build` pass. The build's `pnpm run coverage:check` remains green at `492` wrapper functions, `218` ABI events, `492` HTTP methods, and `260/260` write invariants.
- `pnpm run report:test-gaps` refreshed the persisted report to `235` ready, `223` needs fixture, `51` unsafe on live network, and `201` needs indexer proof items; the dynamic catalog proof remains documented separately because the gap reporter attributes evidence only from explicit item-level references.

Blockers and next steps:

- Real fork receipts now prove `28/260` write methods against their generated event and indexer declarations. Add deterministic fixtures and persisted receipt evidence for the remaining `232` methods before calling every write path proven end to end.
- Trace-aware nested attribution is implemented and fail-closed, but the configured production RPC path still needs an explicit `debug_traceTransaction`/`callTracer` capability proof (or a dedicated trace endpoint) before nested shared-signature events can be guaranteed to project rather than remain raw candidate evidence.

### ABI-Driven Gap Report Evidence

The 2026-08-10 Phase 1 artifact inventories all `33` facets, `492` functions, and `218` ABI event occurrences (`710` total items). Mechanical parity is present for all `710` ABI/RPC items; `709` items have reviewed HTTP entries because the legacy overloaded `ProposalFacet.propose(string,string,address[],uint256[],bytes[],uint8)` variant remains intentionally excluded. The report currently classifies `235` items as `ready`, `223` as `needs fixture`, `51` as `unsafe on live network`, and `201` as `needs indexer proof`, with zero `needs contract change` or `needs API guard` findings. Red-team-attributed proof is present on `30` items and explicit item-level indexer proof on `17`; the broader dynamic catalog and receipt artifacts remain documented separately because the gap reporter only promotes source-attributable item evidence.
