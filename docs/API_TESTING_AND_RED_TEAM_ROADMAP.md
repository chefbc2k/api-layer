...
| Event and indexer projection proof | Blocked — `60/260` cumulative real writes proven on 2026-08-25 | generated-registry decode, PostgreSQL projection, replay/reorg/rollback resilience, and deterministic receipts for the remaining `200` writes; `VoiceAssetFacet.registerVoiceAssetForCaller` is mounted publicly but contract-restricted to an unreachable diamond self-call |
| Local-fork destructive automation | Complete and verified (2026-08-25) | deterministic `10/10`-stage cold run on this branch, funded/approved/aged fixtures, `18/18` HTTP contract proof, `38` real receipts indexed into PostgreSQL, `446/446` reviewed read/event attempts with structured state gaps, bounded Anvil history, strict live flags, `9/9` focused tests, and green TypeScript/lint/build/coverage gates |
...
### Event And Indexer Projection Run — 2026-08-25

Status: **blocked; do not merge this section yet**.

Evidence on `codex/event-indexer-proof`:

- The 2026-08-25 receipt expansion adds `VoiceMetadataFacet.setAnalysisVersion`. The loopback-only HTTP workflow proves the successful receipt and the canonical `AnalysisVersionUpdated` transition log. Contract-source review also corrected its stale post-state metadata: the deployed ABI has no analysis-version getter, so the catalog no longer claims that unrelated geographic data is a valid readback.
- The current cold artifact contains `133` receipts across `60` distinct methods, `203` canonical raw events, and `137` projection rows. The production indexer decoded `AnalysisVersionUpdated` through the generated registry with zero receipt failures, and replay left `raw_events` plus all `22` projection-table counts unchanged. Cumulative real-receipt coverage is therefore `60/260`.
- The 2026-08-24 receipt expansion adds three previously unproven writes: `VoiceMetadataFacet.updateVoiceClassifications`, `VoiceMetadataFacet.updateClassificationCategory`, and `VoiceMetadataFacet.updateGeographicData`. A disposable founder-owned voice asset now proves complete classification replacement, category-specific replacement plus indexed search readback, and bounded geographic metadata with canonical struct readback.
- The runner indexed the three 2026-08-24 additions with zero receipt failures. `VoiceClassificationsUpdated`, `ClassificationCategoryUpdated`, and `GeographicDataUpdated` each decoded through the generated registry as the event declared by its write invariant.
- The same fork exposed a contract/API reachability blocker for `VoiceAssetFacet.registerVoiceAssetForCaller`: the generated HTTP route reaches the deployed selector, but the facet requires `msg.sender == address(this)` and no deployed contract path calls the method. Direct HTTP and EOA attempts deterministically revert with `Only facets via diamond can call`. Its reviewed actor invariant now identifies a diamond self-call instead of a voice-admin/operator role; proving the positive path requires a contract change or removal of the mounted public route.
- The local-fork runner has a tenth `event-indexer-proof` stage. The current `21/21` HTTP contract stage persists all `94` successful receipt-confirmed transaction hashes instead of discarding them; the indexer proof combines that artifact with the lifecycle reports, keeps only successful writes to the configured diamond, attributes each selector through the generated write registry, and ingests each exact receipt block through `EventIndexer` into disposable PostgreSQL with all three migrations applied.
- The latest cold run passed all `10/10` stages on their first attempt and indexed `133` workflow receipts across `60` distinct catalog write methods. The production indexer persisted `203` canonical raw events and `137` projection rows, satisfied every `all`, `one-of`, or `none` event assertion plus every declared projection-table assertion, and preserved all `23` table counts when all proven blocks were replayed.
- Contract-source review exposed a stale catalog expectation for `VoiceDatasetFacet.setMaxAssetsPerDataset`: the implementation changes state without emitting an ABI event. The reviewed invariant now requires receipt success plus `getMaxAssetsPerDataset` readback and explicitly forbids a fabricated projection; a focused receipt-proof regression test covers this eventless path. The `MarketplaceFacet.unpause` readback was also corrected to `isPaused == false`.
- Generated-registry tests decode all `214` addressable registry entries derived from `218` ABI event declarations and bind all `260` write invariants to `285` declared event expectations, `149` projection references, and `28` intentionally eventless writes. The broader synthetic catalog proof remains separate from real-receipt coverage.
...
- Current verification is green: `pnpm run test:write-invariants` passed `5/5`, `pnpm run test:indexer:assurance` passed `57/57` active tests, `pnpm run test:indexer:postgres` passed `4/4`, the isolated metadata receipt proof passed `1/1`, and `pnpm run test:actor-negative-paths` passed `100/100`. With `pnpm` selected from `pnpm-lock.yaml`, the ordered TypeScript, lint, and build gates passed, followed by an explicit `pnpm run coverage:check` at `492` functions, `218` events, `492` HTTP methods, and `260/260` write invariants. `pnpm run test:coverage` passed at `99.70%` statements, `99.45%` branches, `99.54%` functions, and `99.76%` lines.
- The 2026-08-25 loopback orchestrator completed all `10/10` stages on their first attempt. Fixture provisioning, the expanded HTTP receipt stage, marketplace settlement, governance activation/voting, the event-indexer stage, and the exhaustive sweep all passed; the final report contains only the established `16` structured `needs fixture` read/event gaps and no runner failure.

Blockers and next steps:

- Add deterministic fixtures and persisted receipt evidence for the remaining `200` write methods before claiming every write path is proven end to end.
- Resolve `VoiceAssetFacet.registerVoiceAssetForCaller` at the contract/API boundary: add an actual diamond self-call workflow if the method is intentionally internal, or remove the externally mounted HTTP route and reclassify the ABI surface.
- Preserve cumulative receipt evidence across future runner retries so the persistent artifact records the full distinct-method union directly.
...
The actor report, revalidated on 2026-08-25, covers all `259` mounted HTTP write endpoints across `13` domains and lists the intentionally excluded legacy `ProposalFacet.propose(string,string,address[],uint256[],bytes[],uint8)` overload separately. The generated matrix contains `1,813` founder/admin/operator/buyer/seller/licensee/collaborator method cases, `777` unknown-key/read-only-key/signer-mismatch API-boundary cases, and `3,150` missing/stale/revoked/expired/ownership/self/protocol-contract mismatch cases. The reduction from `3,171` reflects the source-reviewed classification of `VoiceAssetFacet.registerVoiceAssetForCaller` as a contract-only diamond self-call rather than a role-gated EOA path. Protected capability mappings explicitly cover commercialization, listing, transfer, minting, voting, upgrades, pauses, recovery, withdrawals, and ownership-controlled mutation.
