...
describe("runCreateMarketplaceListingWorkflow", () => {
  ...
  it("falls back from a transient null listing read before the listing stabilizes", async () => {
    const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout").mockImplementation(((callback: (...args: never[]) => void) => {
      callback();
      return 0;
    }) as typeof setTimeout);
    const marketplace = {
      listAsset: vi.fn().mockResolvedValue({ statusCode: 202, body: { txHash: "0xlist" } }),
      getListing: vi.fn()
        .mockResolvedValueOnce({ statusCode: 500, body: { error: "temporary-null" } })
        .mockResolvedValueOnce({ statusCode: 200, body: { tokenId: "15", price: "1800", isActive: true } }),
      getAssetState: vi.fn().mockResolvedValue({ statusCode: 200, body: "1" }),
      getOriginalOwner: vi.fn().mockResolvedValue({ statusCode: 200, body: "0x00000000000000000000000000000000000000aa" }),
      isInEscrow: vi.fn().mockResolvedValue({ statusCode: 200, body: true }),
      assetListedEventQuery: vi.fn().mockResolvedValue([{ transactionHash: "0xlist-receipt" }]),
    ...
    try {
      const result = await runCreateMarketplaceListingWorkflow({
        addressBook: { toJSON: () => ({ diamond: "0x0000000000000000000000000000000000000ddd" }) },
        providerRouter: {
          withProvider: vi.fn().mockImplementation(async (_mode: string, _label: string, work: (provider: { getTransactionReceipt: (txHash: string) => Promise<unknown> }) => Promise<unknown>) => work({ getTransactionReceipt: vi.fn(async () => ({ blockNumber: 1104 })) })),
        },
      } as never, auth as never, "0x00000000000000000000000000000000000000aa", {
        tokenId: "15",
        price: "1800",
      ...
      expect(result.listing.read).toMatchObject({
        tokenId: "15",
        isActive: true,
      });
      expect(marketplace.getListing).toHaveBeenCalledTimes(2);
  ...
  it("falls back through a null listing read before the stabilized listing appears", async () => {
    const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout").mockImplementation(((callback: (...args: never[]) => void) => {
      callback();
      return 0;
    }) as typeof setTimeout);
    const marketplace = {
      ...
      getListing: vi.fn()
        ...
        .mockResolvedValueOnce({ statusCode: 200, body: { tokenId: "16", price: "1800", isActive: true } }),
      getAssetState: vi.fn().mockResolvedValue({ statusCode: 200, body: "1" }),
      getOriginalOwner: vi.fn().mockResolvedValue({ statusCode: 200, body: "0x00000000000000000000000000000000000000aa" }),
      isInEscrow: vi.fn().mockResolvedValue({ statusCode: 200, body: true }),
      assetListedEventQuery: vi.fn().mockResolvedValue([{ transactionHash: "0xlist-receipt" }]),
    ...
    try {
      const result = await runCreateMarketplaceListingWorkflow({
        ...
        providerRouter: {
          withProvider: vi.fn().mockImplementation(async (_mode: string, _label: string, work: (provider: { getTransactionReceipt: (txHash: string) => Promise<unknown> }) => Promise<unknown>) => work({
            getTransactionReceipt: vi.fn(async () => ({ blockNumber: 1104 })),
          })),
        },
      } as never, auth as never, "0x00000000000000000000000000000000000000aa", {
        tokenId: "16",
      ...
      expect(marketplace.getListing).toHaveBeenCalledTimes(2);
      expect(result.listing.read).toEqual({ tokenId: "16", price: "1800", isActive: true });
    } finally {
      setTimeoutSpy.mockRestore();
    }
  });
  ...
  it("falls back from transient null listing readbacks before the marketplace state converges", async () => {
    const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout").mockImplementation(((callback: (...args: never[]) => void) => {
      callback();
      return 0;
    }) as typeof setTimeout);
    const marketplace = {
      ...
      getListing: vi.fn()
        .mockResolvedValueOnce(null)
        .mockResolvedValueOnce({ statusCode: 200, body: { tokenId: "15", price: "1700", isActive: true } }),
      getAssetState: vi.fn().mockResolvedValue({ statusCode: 200, body: "1" }),
      getOriginalOwner: vi.fn().mockResolvedValue({ statusCode: 200, body: "0x00000000000000000000000000000000000000aa" }),
      isInEscrow: vi.fn().mockResolvedValue({ statusCode: 200, body: true }),
      assetListedEventQuery: vi.fn().mockResolvedValue([{ transactionHash: "0xlist-receipt" }]),
    ...
    try {
      ...
      expect((result.listing.read as Record<string, unknown>).tokenId).toBe("15");
      expect(marketplace.getListing).toHaveBeenCalledTimes(2);
    } finally {
      setTimeoutSpy.mockRestore();
    }
  ...
  it("retries when the listing readback exhausts a null stabilization pass before recovering", async () => {
    const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout").mockImplementation(((callback: (...args: never[]) => void) => {
      callback();
      return 0;
    }) as typeof setTimeout);
    ...
    getListing.mockResolvedValueOnce({ statusCode: 200, body: { tokenId: "15", price: "1501", isActive: true } });
    const marketplace = {
      listAsset: vi.fn().mockResolvedValue({ statusCode: 202, body: { txHash: "0xlist" } }),
      getListing,
      getAssetState: vi.fn().mockResolvedValue({ statusCode: 200, body: "1" }),
    ...
    try {
      ...
      expect(marketplace.getListing).toHaveBeenCalledTimes(21);
      expect(setTimeoutSpy).toHaveBeenCalled();
      expect(result.listing.read).toEqual({ tokenId: "15", price: "1501", isActive: true });
    } finally {
      setTimeoutSpy.mockRestore();
    }
  });
